...
class ObjectDetector:
    ...
    def set_active_source(self, source):
        ...
        if is_synth:
            ...
            # Uploaded video (*.mp4) - strictly preserves existing YOLO11n real edge CV
            if self.mode != "TRAINED_MODEL":
    ...
    def _init_product_detector(self):
        ...
        if trained_path.exists() and trained_path.is_file():
            try:
                ...
                print(f"[Detector] [OK] OPERATING MODE: TRAINED_MODEL (Loaded: {trained_path})")
                return
            except Exception as err:
                print(f"[Detector] [WARNING] Failed loading trained product model '{trained_path}': {err}")

        # Fallback to Demo Simulator
        self.demo_simulator = DemoShelfSimulator()
        self.mode = "DEMO_SIMULATION"
        print(f"[Detector] [INFO] OPERATING MODE: DEMO_SIMULATION (Using demo simulator for synthetic store video).")
    ...
    def detect(self, frame: np.ndarray) -> List[Detection]:
        ...
        # 2. Product Detection
        # A. If custom SKU model is trained and active, run it
        if self.mode == "TRAINED_MODEL" and self.product_detector is not None:
            try:
        ...
        # B. If synthetic demo video ('videos/store.mp4'), use Demo Simulator with YOLO11n (UNTOUCHED)
        elif self.is_synthetic_demo and self.demo_simulator is not None:
            product_dets = self.demo_simulator.detect_products(frame)
            all_detections.extend(product_dets)

        # C. If Live Webcam (Integrated PC webcam 0 or external USB webcams 1, 2, ...), run strictly OWLv2 Zero-Shot
        elif self.is_webcam:
            if self.owlv2_detector is not None:
        ...
        # D. If Uploaded Video, run Universal Real Edge SKU Detector with YOLO11n (UNTOUCHED)
        elif not self.is_synthetic_demo:

...
def create_synthetic_retail_video(output_path="videos/store.mp4", duration_sec=24, fps=30):
    ...
    for frame_idx in range(total_frames):
        ...
        milk_count = 4 if frame_idx < 320 else 0
        ...
        if 120 <= frame_idx:
            if frame_idx < 220:
                t = (frame_idx - 120) / 100
                cx, cy = 20 + t * 860, 300 - t * 140
            elif frame_idx < 300:
                cx, cy = 880, 160
            elif frame_idx < 380:
                t = (frame_idx - 300) / 80
                cx, cy = 880 + t * (970 - 880), 160 + t * (520 - 160)
            else:
                cx, cy = 970, 520
        ...
        cv2.putText(frame, f"CCTV CAM 01 | STAFF AI: 2 ON DUTY | CUSTOMERS: 8 | OCR PRICE AUDIT: ON | TIME: {frame_idx/30:.1f}s", 
                    (20, 708), cv2.FONT_HERSHEY_SIMPLEX, 0.46, (0, 255, 200), 1)
